...
/**
 * This last creates a list of objects stored through ListNode objects. Objects can be added to the list up to the given total capacity
...
public class LinkedAbstractList<E> extends AbstractList<E> {
	...
	/**
	 * Default constructor for a LinkedAbstractList. Assigns the given parameter as the capacity and initializes size with 0 and front with null
	 * @param capacity The total number of ListNodes a LinkedAbstractList will contain
	 */
	 ...
	 * @throws IndexOutOfBoundsException If the given index is greater than size - 1 or less than 0 
	 */
	@Override
	 ...
	 * @param element The object being placed in the AbstractList in place of the other object
	 * @return The element being replaced from the given index
	 */
	public E set(int idx, E element) {
		if (idx < 0 || idx >= this.size()) throw new IndexOutOfBoundsException("Invalid index");
		else if (element == null) throw new NullPointerException("element cannot equal null");
		else {
			ListNode current = front;
			while (current != null) {
				if (current.data.equals(element)) throw new IllegalArgumentException("Duplicate element");
				current = current.next;
			}
			current = front;
			if (idx == 0) {
				E removedElement = front.data;
				if (size == 1) front = new ListNode(element);
				else front = new ListNode(element, front.next);
				...
				if (idx == size - 1) current.next = new ListNode(element);
				else current.next = new ListNode(element, current.next.next);
				return removedElement;
			}
		}
	}
	 ...
	 * @throws IllegalArgumentException If a duplicate of the element is present or the size == capacity
	 * @throws IndexOutOfBoundsException If the index is less than 0 or greater than size - 1
	 */
	@Override
	public void add(int idx, E element) {
		if (idx < 0 || idx > this.size()) throw new IndexOutOfBoundsException("Invalid index");
		else if (this.size() == capacity) throw new IllegalArgumentException("Size == Capacity");
		else if (element == null) throw new NullPointerException("element cannot equal null");
		else {
			ListNode current = front;
			while (current != null) {
